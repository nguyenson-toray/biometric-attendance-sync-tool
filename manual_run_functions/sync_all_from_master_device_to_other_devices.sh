...
PYTHON_SCRIPT="$SCRIPT_DIR/sync_all_from_master_device_to_other_devices.py"
LOG_DIR="$SCRIPT_DIR/logs"
LOG_FILE="$LOG_DIR/sync_master_to_devices.log"
ERROR_LOG="$LOG_DIR/sync_master_to_devices_error.log"
LOCK_FILE="/tmp/sync_master_to_devices.lock"
...
check_prerequisites() {
    ...
    if ! python3 -c "import pyzk, requests, unidecode" &> /dev/null; then
        log_message "ERROR" "Required Python modules not installed. Run: pip install pyzk requests unidecode"
        exit 1
    fi
...
COMMANDS:
    sync            Sync all users from master device to target devices (default)
...
run_sync() {
    ...
    log_message "INFO" "Starting fingerprint sync from master device to target devices..."
...
run_test() {
    log_message "INFO" "Running connectivity and configuration test..."
    
    if [ -f "$SCRIPT_DIR/test_fingerprint_sync.py" ]; then
        if [ "$VERBOSE" = "true" ]; then
            python3 "$SCRIPT_DIR/test_fingerprint_sync.py" 2>&1 | tee -a "$LOG_FILE"
        else
            python3 "$SCRIPT_DIR/test_fingerprint_sync.py" >> "$LOG_FILE" 2>&1
        fi
        ...
        log_message "WARN" "Test script not found: $SCRIPT_DIR/test_fingerprint_sync.py"
        log_message "INFO" "Running basic connectivity check instead..."
...
for device in config.devices:
    print(f'Testing target device: {device[\"device_id\"]} ({device[\"ip\"]})')
    zk = ZK(device['ip'], port=4370, timeout=5)
    try:
        conn = zk.connect()
        if conn:
            ...
            print(f'❌ {device[\"device_id\"]} failed to connect')
    except Exception as e:
